...
/* enums */
enum { CurNormal, CurHand, CurResizeTL, CurResizeTR, CurResizeBL, CurResizeBR, CurMove, CurLast }; /* cursor */
...
{
	int keepaspectratio = arg->i;
	float ratio;
	int ocx, ocy, nw, nh, x, y;
	int ocx2, ocy2, nx, ny;
	Client *c;
	Monitor *m;
	XEvent ev;
	...
	vertcorner  = ny < c->h / 2;
	switch(2*horizcorner + vertcorner) {
		case 0:
			resizecursor = cursor[CurResizeBR];
			break;
		case 1:
			resizecursor = cursor[CurResizeTR];
			break;
		case 2:
			resizecursor = cursor[CurResizeBL];
			break;
		case 3:
			resizecursor = cursor[CurResizeTL];
			break;
		default:
			die("resizemouse -  switch cursor - unreachable");
	}
	if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
		None, resizecursor->cursor, CurrentTime) != GrabSuccess)
		return;
	...
	do {
		...
		case MotionNotify:
			...
			nw = MAX(x - ocx - 2 * c->bw + 1, 1);
			nh = MAX(y - ocy - 2 * c->bw + 1, 1);
			nx = horizcorner ? x : c->x;
			ny = vertcorner ? y : c->y;
			nw = MAX(horizcorner ? (ocx2 - nx) : (x - ocx - 2 * c->bw + 1), 1);
			nh = MAX(vertcorner ? (ocy2 - ny) : (y - ocy - 2 * c->bw + 1), 1);
